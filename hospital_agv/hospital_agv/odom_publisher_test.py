...
class OdomPublisher(Node):
    def __init__(self):
        ...
        self.prev_pose.orientation.w = 1.0 #prev = previouse\
        ...
        self.odom_pub = self.create_publisher(Odometry, '/agv/odom', 10) #agv/odom, name of the topic
        ...
        self.timer_readWheel = self.create_timer(1/5, self.wheel.readInfo) #read infor fun 5 time in 1s, 
        self.timer_controlWheel = self.create_timer(1/20, self.controlWheel) #1/20, get by trail and error
    ...
    def printInfo(self):
        ...
        self.cmdCalOdom = self.RPM_to_velocity(self.cmdL, self.cmdR, self.L, self.wheel.radius*2) #send the cmd in m/s
...
class Controller():
    ...
    def __init__(self, port='/dev/ttyUSB0'):
        self.left_id = 2  #assign id2 to the left motor
        self.right_id = 1 #assign id1 to the right motor
    ...
    def readThread(self): # to read the feedback while running other code 
        while (self.run):
            cNum = self.serial.inWaiting() #
            if (cNum > 0):
                data_str = self.serial.read(cNum) #
    ...
    def readThreadOld(self):
        while (self.run):
            ...
            if (cNum > 0):
                #print(cNum)
                data_str = self.serail.read(cNum)
    ...
    def readInfo(self):
        ...
        time.sleep(0.1)
        self.serial.write(cmdL)
        time.sleep(0.1)
        self.serial.write(cmdR)
